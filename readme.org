...
this set of scripts allows deploying a variety of /UNIX-like/ (**nix*) environments
with tiny personal configurations for my set-up to work as intended.

since this is for a general usage, /POSIX/-compliance should be a must; something
as simple as installing a base system should not require the use anything that
isn't shipped already with the system.

if it's not clear already, this file itself is the actual code of the various
scripts, by using [[https://www.gnu.org/software/emacs/][GNU/Emacs']] [[https://orgmode.org/worg/org-contrib/babel/intro.html][Org-Mode's Babel feature]] this file can be /tangle/'d
to produce the code (even for multiple files)
...
git_dots_install() {
    # variables
    repo="https://gitlab.com/anntnzrb/_nixrice.git"
    ...
    # copy the .files to ~
    cp -fr "$clone_dir/." "$HOME/"

    # cleanup
    rm -fr "$clone_dir" "$HOME/.git/" "$HOME/LICENSE" "$HOME/readme.md"
...
git_src_install() {
    # variables
    prg_name=`basename "$1" .git`
    clone_dir="$HOME/temp/$prg_name"
    git clone --depth '1' "$1" "$clone_dir" || err 'Could not clone'
    { make -C "$clone_dir" && make -C "$clone_dir" install; } || err 'Could not install'
